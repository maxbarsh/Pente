...
/**
 * This class contains the main method that should be run to execute the program.  
 * 
 * This class is the container for both logic and GUI. It will hold PenteWindow,
 * while also handling a portion of game logic itself.
 */

public class GamePente {
	/**
	...
	public static void main ( String[] args ) {
		GamePente game = new GamePente();

	}
	
	/**
	 * Default constructor for GamePente. Calls the init() method,
	 * which should do two things:
	 * 	-build and display the start menu.
	 * 	-prep the Board JPanel.
	 */
	public GamePente () {
		init();
	}
	...
	/**
	 * When called, this method effectively resets the state of the game.
	 * 
	 * To do this:
	 * 	-cap counts must be reset
	 * 	-go through the entire board, setting each status to 0.
	 * 	-call Board.update()
	 */
	public void reset () {
		winDetected = false;
	...
	/**
	 * This method is used to set the listener for each of the JButtons in the start menu to be pointer.
	 */
	...
	/**
	 * This method is used to set the listener for each of the JPanels on the PlayPane.
	 */
	...
	public class ClickListener implements MouseListener, ActionListener {
		...
		/**
		 * This mouseClicked method contains most of the sequential checking.
		 * Makes several calls to PenteUtil to perform various checks.
		 * 
		 * Also used for other types of ActionEvents with other types of components,
		 * such as JMenuItems and JButtons
		 */
		public void mouseClicked ( MouseEvent arg0 ) {
			...
			/**
			 * if things happen the way they should and the clicked JPanel is a
			 * Stone on the Board, then getName() should return something of the
			 * form: 1,2 OR 4,2 where the first number is the x, and the 2nd is the
			 * y.
			 */
			...
			Object[] keys = PenteUtil.getNonEmptyNeighbors(x,y,gui.getPane()).keySet().toArray();
			
			/**
			 * CHECK CAPTURES, THEN PERFORM NECESSARY UPDATES TO CAPCOUNT AND
			 * BOARD.
			 */
			...
			case 1:
				...
				gui.setMessage(turnStatus,
				           "Player " + move + " has a tria!",whiteCaps,blackCaps);
				break;

			case 2:

				gui.setMessage(turnStatus,
				           "Player " + move + " has a tessera!",whiteCaps,blackCaps);
				break;

			default:

				gui.setMessage(turnStatus,
				           "Player " + move + " has gone.",whiteCaps,blackCaps);
				break;
			}
		...
		/**
		 * this method deals solely with the JMenuItems while playing, as well as the 
		 * "New Game" button on NewGameDialog and the "New Game" button on the WinDialog.
		 */
